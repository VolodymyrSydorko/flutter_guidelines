...
class AnimatedSplashScreen extends StatefulWidget {
  ...
  factory AnimatedSplashScreen({
    ...
    required PageRouteInfo nextRoute,
  ...
  factory AnimatedSplashScreen.withScreenFunction({
    ...
    int duration = 2500,
    required dynamic splash,
    required Image backgroundSplash,
...
class _AnimatedSplashScreenState extends State<AnimatedSplashScreen>
  ...
  navigator(screen) {
    Future.delayed(Duration(milliseconds: w.duration < 100 ? 100 : w.duration))
        .then((_) {
      context.replaceRoute(screen);
    });
  }

  /// Return icon of splash screen
  Widget getSplash() {
    final size = w.splashIconSize ?? MediaQuery.of(context).size.shortestSide;
    ...
    return getTransition(
      child: main(
        child: Stack(
          children: [
            w.backgroundSplash != null
                ? Positioned.fill(child: w.backgroundSplash!)
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: widget.backgroundColor, body: getSplash());
  }
}
